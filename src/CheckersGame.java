...
public class CheckersGame extends AbstractGame {
    ...
    public CheckersGame() {
        ...
        this.turn = this.p1;
        this.setJumps(new JumpsList());
    }

    private JumpsList jumps;

    @Override
    public int[] scanForMove() { // returns in index format (0..7)
        int[] move;

        if (!jumps.getJumps().isEmpty()) {
            move = generateJumpMove();
        } else {
    ...
    private int[] generateJumpMove() {
        ...
        if (jumps.getNumJumps() == 1) {
            ...
            allMoves = appendMoveArray(move, keepJumping(move, new int[] {move[2], move[3]}));
    ...
    public JumpsList getJumps() {
        return jumps;
    }

    public void setJumps(JumpsList jumps) {
        this.jumps = jumps;
    }
